...
public class AdminController {
	...
	public ModelAndView countPage() {
		...
		List<User> newUser;
		if(countUser > 5) newUser = users.subList(0, 5);
		else newUser = users;
		List<Posts> newPost; 
		if (countPost>5) newPost = posts.subList(0, 5);
		else newPost = posts;
		List<com.SocialNetwork.Entity.LoggedInCount> newLog;
		if(countLogged > 5) newLog = loggedIn.subList(0, 5);
		else newLog = loggedIn;
	...
	public ModelAndView postManagePage() {
		...
		for(Posts p:posts) {
			...
			for(Actions a:actions) {
				if(a.getType().equals("like")) numLike++;
				else if(a.getType().equals("comment")) numComment++;
			}
